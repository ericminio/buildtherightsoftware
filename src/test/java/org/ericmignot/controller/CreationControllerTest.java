...
import static org.ericmignot.util.SpecMatcher.aSpec;
...
public class CreationControllerTest {
	...
	createASpecWithDefaultContent() {
		...
		verify( repo ).saveSpec( argThat( aSpec().withTitle( "toto" )
												 .withContent( readFile( "target/html/newSpecContent.html" ) ) ) );
		verify( renderer ).setSpec( argThat( aSpec().withTitle( "toto" ) ) );
